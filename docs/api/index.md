---
title: Recova Backend API Index
description: Indeks dokumentasi kontrak API publik, standar request dan response, serta kompatibilitas API Recova Backend.
owner: backend-owner
reviewers:
  - engineering-lead
  - platform-docs-maintainer
doc_status: draft
source_repo: recova-backend-v2
source_path: docs/api/index.md
last_reviewed: 2026-05-08
---

# Recova Backend API

Halaman ini memetakan dokumentasi kontrak API layanan backend Recova yang dikonsumsi client aplikasi.

## API Documentation Scope

- inventaris endpoint publik,
- kontrak request dan response,
- error contract dan status code,
- aturan kompatibilitas perubahan endpoint.

## Core API Documents

- [API Reference](/Users/macbookpro/Development/recova-backend-v2/docs/api-reference.md)
- [API Compatibility Matrix](/Users/macbookpro/Development/recova-backend-v2/docs/api-compatibility-matrix.md)
- [API Response Standard](/Users/macbookpro/Development/recova-backend-v2/docs/api-response-standard.md)
- [API Versioning Standard](/Users/macbookpro/Development/recova-backend-v2/docs/standards/api-versioning.md)

## Related Sections

- [Documentation Overview](/Users/macbookpro/Development/recova-backend-v2/docs/overview.md)
- [Database](/Users/macbookpro/Development/recova-backend-v2/docs/database.md)
- [Modules](/Users/macbookpro/Development/recova-backend-v2/docs/modules/index.md)
- [Integrations](/Users/macbookpro/Development/recova-backend-v2/docs/integrations/index.md)
