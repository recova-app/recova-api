---
title: Recova Backend Operations Index
description: Indeks dokumentasi operasional untuk menjalankan, memantau, dan memulihkan layanan Recova Backend.
owner: backend-owner
reviewers:
  - engineering-lead
  - platform-docs-maintainer
doc_status: draft
source_repo: recova-backend-v2
source_path: docs/operations/index.md
last_reviewed: 2026-05-08
---

# Recova Backend Operations

Halaman ini memetakan dokumen operasi harian layanan, termasuk observability, incident handling, deployment readiness, dan health checks.

## Operations Documentation Scope

- runbook operasional,
- health/readiness contract,
- logging, metrics, dan tracing baseline,
- verifikasi sebelum release.

## Core Operations Documents

- [Health and Readiness](/Users/macbookpro/Development/recova-backend-v2/docs/operations/health-readiness.md)
- [Database Migrations](/Users/macbookpro/Development/recova-backend-v2/docs/operations/database-migrations.md)
- [Database Seeding](/Users/macbookpro/Development/recova-backend-v2/docs/operations/database-seeding.md)

## Related Sections

- [Architecture](/Users/macbookpro/Development/recova-backend-v2/docs/architecture.md)
- [Standards](/Users/macbookpro/Development/recova-backend-v2/docs/standards/index.md)
- [Integrations](/Users/macbookpro/Development/recova-backend-v2/docs/integrations/index.md)
- [Roadmap](/Users/macbookpro/Development/recova-backend-v2/docs/roadmap/index.md)
